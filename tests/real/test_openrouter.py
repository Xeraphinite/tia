...
async def test_luna_runs_multi_tool_scenario_and_session_followup() -> None:
    ...
    first = await runner.run(
        ...
        user_input=(
            "You must use the weather tool to check Shanghai, then use the todo tool to add "
    ...
    assert {"weather", "todo"} <= first_tools
